import fs from 'fs';
// PERHATIAN: output = daily-task/daily-task.html (file kanonis). Jangan jalankan
// kecuali template diubah — file kanonis punya tombol admin manual yg tidak ada di template.
...
const tail = `
    <script type="module">
          ...
          .map(
            (p) => {
              ...
              return \`<a class="item active-modul" href="\${href}">
                <div class="icon-tile">
                  <img data-slug="\${slug}" src="image-daily-task/\${slug}/1.jpg" alt="\${name}" onerror="dtProbe(this)" />
                </div>
                <span>\${name}</span>
              </a>\`;
            }
      ...
      function dtProbe(img) {
        ...
        const folder = 'image-daily-task/' + slug + '/';
...
fs.writeFileSync('daily-task/daily-task.html', full, 'utf8');
