...
const DATA_FILE = path.join(process.cwd(), 'scripts', 'template-menu-baru.json');

async function syncNewMenus() {
  if (!fs.existsSync(DATA_FILE)) {
    console.log('❌ File scripts/template-menu-baru.json tidak ditemukan!');
    console.log('→ Buat file scripts/template-menu-baru.json dengan format JSON');
    process.exit(1);
  }
  ...
  console.log(`Found ${newItems.length} item(s) di template-menu-baru.json`);
