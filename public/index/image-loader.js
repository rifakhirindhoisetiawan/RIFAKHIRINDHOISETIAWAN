...
export async function loadIndexPhoto(menuFolder, imgEl) {
  const folder = `image-index/${menuFolder}/`;
...
// Loader untuk image-kopi (local files) - pakai background-image + fallback ke image-index biar tidak emot
export async function loadJaDiPhoto(slug, tileEl) {
  if (tileEl.dataset.hasimg === '1') return; // sudah ada gambar upload
  
  const kopiBase = location.pathname.includes('/modul/') ? '../kopi/image-kopi/' : 'image-kopi/';
  const folder = `${kopiBase}${slug}/`;
  const urls = generateUrls(folder);
  
  let workingUrl = await findFirstWorkingUrl(urls);
  
  // Fallback ke image-index kalau kopi kosong (biar tidak emot)
  if (!workingUrl) {
    const altFolder = `../index/image-index/${slug}/`;
    const altUrls = generateUrls(altFolder);
    workingUrl = await findFirstWorkingUrl(altUrls);
  }
  // Fallback terakhir ke thumbnail grid (pindah ke grid/image-index.jpg)
  if (!workingUrl) {
    const defUrl = '../grid/image-index.jpg';
    workingUrl = await loadSingleImage(defUrl).then(() => defUrl, () => null);
  }
