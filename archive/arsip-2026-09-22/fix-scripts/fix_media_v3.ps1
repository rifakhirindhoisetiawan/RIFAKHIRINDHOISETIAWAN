...
# Find the end of the media query by matching braces
$brace = 0
$end = 0
...
if ($end -eq 0) { Write-Host 'End not found'; exit 1 }
Write-Host "Found: $($end - $i) chars"
...
if ($content.Contains($oldMedia)) {
    ...
    Write-Host 'Old media not found'
    Write-Host "Expected length: $($oldMedia.Length)"
